<?php
    function removeReply($id, $auth) {
        ...
        $stm = $db->prepare("SELECT id FROM reply WHERE user_id=:user_id AND id=:id");
        $stm->bindValue("id", $id);
        $stm->bindValue("user_id", $user_id);
        $result = $stm->execute();
        if($values = $result->fetchArray(SQLITE3_ASSOC)) {
            $stm = $db->prepare("DELETE FROM reply WHERE id=:id");
            $stm->bindValue("id", $id);
            $stm->execute();
        }
    }
    ...
    if(array_key_exists("auth", $_COOKIE) && array_key_exists("id", $_POST)) {
        removeReply($_POST["id"], $_COOKIE["auth"]);    
    }
?>